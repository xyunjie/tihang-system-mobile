...
let PAGE_X = 0; let PAGE_Y = 0; let T_PAGE_X = 0; let T_PAGE_Y = 0
let CUT_L = 0; let CUT_T = 0; let CUT_R = 0; let CUT_B = 0
let IMG_RATIO = 1; let IMG_REAL_W = 0; let IMG_REAL_H = 0
...
// 拖动移动
function contentMoving(e: any) {
  let dragLengthX = (PAGE_X - e.touches[0].pageX) * DRAFG_MOVE_RATIO
  let dragLengthY = (PAGE_Y - e.touches[0].pageY) * DRAFG_MOVE_RATIO

  if (dragLengthX > 0) {
    if (cutL.value - dragLengthX < 0)
      dragLengthX = cutL.value
  }
  else {
    if (cutR.value + dragLengthX < 0)
      dragLengthX = -cutR.value
  }

  if (dragLengthY > 0) {
    if (cutT.value - dragLengthY < 0)
      dragLengthY = cutT.value
  }
  else {
    if (cutB.value + dragLengthY < 0)
      dragLengthY = -cutB.value
  }
...
function dragMove(e: any) {
  ...
  switch (dragType) {
    ...
    case 'left':
      ...
      if (CUT_L - dragLengthL < 0)
        break
      if ((CUT_L - dragLengthL) > (cropperW.value - cutR.value))
        break
    ...
    case 'top':
      ...
      if (CUT_T - dragLengthT < 0)
        break
      if ((CUT_T - dragLengthT) > (cropperH.value - cutB.value))
        break
    ...
    case 'rightBottom':
      ...
      if (CUT_B + dragLengthY < 0 || CUT_R + dragLengthX < 0)
        break
...
<template>
  <view v-if="visible" class="fixed inset-0 z-50 bg-black">
    <!-- 顶部导航栏 -->
    <view
      class="fixed left-0 right-0 top-0 z-10 flex items-center justify-between bg-black px-4 text-white"
      :style="{
    ...
    >
      <view class="text-base" @click="handleCancel">
        取消
      </view>
      <view class="text-base">
        裁剪图片
      </view>
      <view class="text-base text-blue-400" @click="handleConfirm">
        完成
      </view>
    </view>
          ...
          >
            ...
            >
              ...
              <view class="relative h-full w-full">
                <!-- 九宫格辅助线 -->
                <view class="absolute h-full w-px border-white border-opacity-30" :style="{ left: '33.33%' }" />
                <view class="absolute h-full w-px border-white border-opacity-30" :style="{ left: '66.66%' }" />
                <view class="absolute h-px w-full border-white border-opacity-30" :style="{ top: '33.33%' }" />
                <view class="absolute h-px w-full border-white border-opacity-30" :style="{ top: '66.66%' }" />

                <!-- 边框拖拽线 -->
                <view class="absolute h-1 w-full cursor-pointer -top-1" data-drag="top" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-full w-1 cursor-pointer -right-1" data-drag="right" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-1 w-full cursor-pointer -bottom-1" data-drag="bottom" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-full w-1 cursor-pointer -left-1" data-drag="left" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />

                <!-- 角点控制点 -->
                <view class="absolute left-1/2 h-3 w-3 transform cursor-pointer border-2 border-gray-300 bg-white -top-1.5 -translate-x-1/2" data-drag="top" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-3 w-3 cursor-pointer border-2 border-gray-300 bg-white -right-1.5 -top-1.5" data-drag="topRight" />
                <view class="absolute top-1/2 h-3 w-3 transform cursor-pointer border-2 border-gray-300 bg-white -right-1.5 -translate-y-1/2" data-drag="right" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-3 w-3 cursor-pointer border-2 border-gray-300 bg-white -bottom-1.5 -right-1.5" data-drag="rightBottom" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute left-1/2 h-3 w-3 transform cursor-pointer border-2 border-gray-300 bg-white -bottom-1.5 -translate-x-1/2" data-drag="bottom" @touchstart.stop="dragStart" @touchmove.stop="dragMove" @touchend.stop="dragEnd" />
                <view class="absolute h-3 w-3 cursor-pointer border-2 border-gray-300 bg-white -bottom-1.5 -left-1.5" data-drag="bottomLeft" />
                <view class="absolute top-1/2 h-3 w-3 transform cursor-pointer border-2 border-gray-300 bg-white -left-1.5 -translate-y-1/2" data-drag="left" @touchstart.stop="dragStart" @touchmove.stop="dragMove" />
                <view class="absolute h-3 w-3 cursor-pointer border-2 border-gray-300 bg-white -left-1.5 -top-1.5" data-drag="leftTop" />
              </view>
            </view>
          </view>
    ...
    <!-- 底部操作栏 -->
    <view
      class="fixed bottom-0 left-0 right-0 z-10 bg-black px-4 py-4"
      :style="{ paddingBottom: `${safeAreaInsetBottom}px` }"
    >
      <view class="flex items-center justify-around">
        <view
          class="rounded-lg bg-gray-700 px-6 py-2 text-sm text-white active:bg-gray-600"
          @click="chooseImage"
        >
          重新选择
        </view>
        <view
          class="rounded-lg bg-blue-500 px-6 py-2 text-sm text-white active:bg-blue-600"
          @click="handleConfirm"
        >
          确认上传
        </view>
