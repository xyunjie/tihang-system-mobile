...
function getInitialSystemTheme(): 'light' | 'dark' {
  ...
  try {
    ...
    if (mpTheme === 'dark' || mpTheme === 'light') return mpTheme
    ...
    // #ifdef APP-PLUS
    if (info?.appTheme === 'dark' || info?.appTheme === 'light') return info.appTheme
    if (info?.appTheme === 'auto') {
      try {
        const dev = uni.getDeviceInfo?.() as any
        const osTheme = dev?.osTheme
        if (osTheme === 'dark' || osTheme === 'light') return osTheme
      }
      catch (e3) {
        // ignore
      }
    ...
    if (h5Theme === 'dark' || h5Theme === 'light') return h5Theme
    // #endif
  }
  catch (e) {
    // ignore
